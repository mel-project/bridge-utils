...
pub mod test_mod {
    ...
    pub struct Test<M: Clone>(ethers::contract::Contract<M>);
    impl<M: Clone> Clone for Test<M> {
        fn clone(&self) -> Self {
            Test(self.0.clone())
        }
    }
    impl<M: Clone> std::ops::Deref for Test<M> {
        type Target = ethers::contract::Contract<M>;
        fn deref(&self) -> &Self::Target {
            &self.0
        }
    }
    impl<M: ethers::providers::Middleware + Clone> std::fmt::Debug for Test<M> {
    ...
    impl<M: ethers::providers::Middleware + Clone> Test<M> {
    ...
    impl<M: ethers::providers::Middleware + Clone> From<ethers::contract::Contract<M>> for Test<M> {
